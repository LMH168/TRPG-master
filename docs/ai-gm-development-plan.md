...
- 状态：Ready，优先完成四个预设模组；原文、署名和来源随仓库归档
...
| 哪些内容可以提交 GitHub | 四份预设原文、manifest、catalog、结构化运行包、代码和测试均进入项目仓库 | manifest 保留作者、来源、许可状态和哈希；CoC7 规则书仍仅留本地 |
...
首阶段目标是把四个预设模组做成可运行、可验收的 ModulePack，不处理用户上传和自动解析。项目方已确认四份预设完整原文、结构化改编数据和来源片段随当前 GitHub 仓库归档。manifest 与预设选择页记录作者、来源、版本和资料状态，这些字段只用于署名与追溯，不作为加载、开局、Context Builder 或 AI 主持的拦截条件。
...
开发前置资料已集中到后端目录并进入 Git；《追书人》已经编译为 ModulePack v2，其余三份当前仍只完成归档和完整性记录：
...
每个模组和规则集目录都有 `manifest.json`、SHA-256 和当前状态；模组原文进入 Git，CoC7 规则书仍只留本地。原始 PDF/DOC/DOCX 是可追溯开发输入，不能直接作为 Kernel 状态源，也不会整本放入生产 Prompt；Context Builder 只选择运行包中已标注权限的少量片段。
...
- 增加原始文件元数据、ModulePack manifest、版本、哈希、来源和可见性记录；四份模组原文进入仓库，但不进入玩家投影。
...
四个预设的完整原文和开发安装包进入当前项目仓库并供测试房间运行。每个 manifest 至少记录作者、来源、资料状态和内容哈希；未知信息明确写为“未确认”，不能伪造作者或来源。`license_status` 和说明字段仅供展示和追溯，不参与任何运行时权限判断。
