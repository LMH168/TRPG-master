...
`trpg-backend/modules/presets/` 并随 Git 归档；规则书 PDF 仅在本机
`trpg-backend/rulesets/coc7/source/` 保留并被 Git 忽略。
...
| 模组能否提交 GitHub | 项目方已确认四份预设原文和结构化改编包进入当前仓库 | manifest 保留作者、来源、许可状态与哈希；产品公共下载渠道另行管理 |
...
本项目的实现以中文可审查性为硬要求：新增或修改的关键代码必须添加中文注释；每个函数、方法和异步任务必须在定义处提供中文文档字符串，说明职责、输入、输出和失败行为；系统提示词、Agent 指令、结构化说明和模型错误提示默认使用中文。技术标识符、API 字段名、协议枚举和数据库列名保持稳定英文名称。

注释应解释设计决策、边界条件以及 Kernel、权限、幂等和恢复约束，不重复描述显而易见的赋值或循环。测试名称、测试注释和失败断言优先使用中文；任何 Prompt、魔法数字或默认降级策略都不能成为没有来源说明的隐式行为。每个 PR 的自检必须确认新增函数和关键分支具备足够中文说明，便于人工 Review。
...
四个预设模组是当前首要交付对象。开发阶段先制作人工校对的运行包，并在模组选择页展示作者、来源链接和资料说明；不实现用户上传或自动解析。四份完整原文、结构化改编数据和来源片段均可进入当前仓库，资料状态只用于展示和追溯，不参与运行时拦截。
...
| 对象 | 作用 |
| --- | --- |
| `manifest` | 模组身份、版本、规则集、人数、作者来源、内容预警、哈希 |
...
```text
manifest          模组、内容版本、规则版本、作者来源和 capability
...
四份预设优先以仓库内可审查的 JSON/YAML 运行包维护，不把“自动读 Word/PDF”放在游戏内核之前。每个预设至少固定 `preset_id`、`content_version`、支持人数、作者来源、规则版本和来源哈希；房间创建后冻结这些值。
...
10. 四份预设原文由项目方确认进入当前仓库，运行时权限不能阻止 AI 获取必要片段；CoC7 规则书仍只作本地参考。对外公共下载时继续展示并核对作者、来源和许可状态。
...
CoC7 原始规则书只归档在开发者本机的 `trpg-backend/rulesets/coc7/source/`，其 SHA-256 和资料状态记录在同目录 `manifest.json`；规则书 PDF 不进入 Git。规则书用于核对规则语义、设计结构化规则和编写测试。四个预设完整原文则归档在 `trpg-backend/modules/presets/*/source/` 并进入 Git；运行时仍读取结构化规则和经过受众过滤的少量 `source_fragments`，不把整份文件直接交给模型或 Kernel。
...
仓库中的 `trpg-backend/modules/presets/追书人/runtime.json` 是当前 Kernel 消费的结构化运行包入口。它保存可审查的运行数据，以及带页码、权限和触发条件的必要原文片段；完整 PDF 同目录归档但不会整本进入模型。完整发布仍须逐条通过下列脚本、真实模型和浏览器门禁。
...
本报告还用《追书人》《银之锁》《林隙的罪恶》《坨子岛》的模组正文和《克苏鲁的呼唤 守秘人规则书 40 周年纪念版》做了能力校验，抽取的是设计约束而不是正文中的主持指令或叙事文本：先声明检定目标、日常行动免检、强推与幸运的限制、灵感检定的关键线索恢复、战斗回合的弹性时间、追逐的抽象地点、SAN 的多阶段状态、状态化解谜、动态物品约束、逐玩家投影、声音与巡逻，以及缺少完整 NPC 数据时的临时战斗模板。当前优先交付四个预设的可运行 ModulePack；用户上传和自动解析后置。四份预设原文及改编数据进入当前仓库，作者、翻译、图片来源和资料状态作为元数据保留，不形成运行时拦截。
