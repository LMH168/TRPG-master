...
class LegacyNarrativeTurnDecisionClient:
    """模拟 DeepSeek 仍返回旧版纯叙事单动作格式。"""
...
async def test_legacy_narrative_decision_is_normalized_without_persistent_effects() -> None:
    """旧版纯叙事格式应恢复普通对话，但不能生成任何持久结果。"""
    module = load_paper_chase()
    state = conversation_state(module)
    store = InMemoryEngineStore()
    store.register_room(module_content=module, initial_state=state)
    player_input = PlayerInput(
        ...
        player_id="player_1",
        actor_id="actor_1",
        client_action_id="legacy-narrative",
        utterance="现在我们该做些什么",
    )
    view = await PlayerViewProjector(RuleEngineService(store)).project(player_input)

    decision = await PromptHostTurnDecisionModel(LegacyNarrativeTurnDecisionClient()).generate(
        HostAgentContext(
            player_input=player_input,
            player_view=view,
            recent_history=RecentTurnContext.empty(
    ...
    assert isinstance(decision, SingleActionDecision)
    assert decision.adjudication.persistence_intent == "none"
    assert decision.adjudication.target.id == view.scene.id
    assert [effect.type for effect in decision.adjudication.success_effects] == ["narrative_only"]
