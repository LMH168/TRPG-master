...
边界：它只进入 planner 与步骤裁决两处模型调用，不进入 `NarrationContext`，
...
| `NarrationContext` | `player_input`, `plan_id`, `plan_goal`, `termination_status`, `completed_steps`, `player_view`, `recent_history`, `memory_context`, `focus_entity_ids`, `active_interaction_entity_ids`, `interaction_source_turn_id`, `interaction_continuity`, `opening_world_time`, `allowed_evidence_refs`, `narration_evidence`, `blocked_step_goal`, `remaining_step_goals`, `player_safe_failure_reason`, `narration_retry_hint` | 单动作和 ActionPlan 共用的玩家安全叙事上下文；`memory_context` 必须与最终 PlayerView 同作用域、同 revision，持久事实仍只来自最终 PlayerView 和已提交 evidence；连续交互字段只约束当前可见 NPC 的近期对话焦点，不进入 GameState |
...
| 模型 | 字段 | 语义 |
|---|---|---|
| `HostAgentContext` | `player_input`, `player_view`, `recent_history`, `memory_context`, `keeper_capabilities`, `agenda_continuation_candidates` | 只接收可信 `PlayerInput`、player-safe `PlayerView`、同作用域近期历史与长期记忆；`room_id/player_id/actor_id/revision` 必须逐项一致。`keeper_capabilities` 是可选的受控 `KeeperCapabilityView`（见 4.3），必须与 `PlayerView` 同 revision；`agenda_continuation_candidates` 只发布服务端限定的玩家安全等待选项，不包含下一 Rule 游标或 Effect |
...
| `plot_threads` | 按 thread_id 保存的权威剧情线程状态、版本和最后 transition event；只允许已发布 Rule 推进 |
...
每个阻塞步骤或连续 Effect 段在 Engine 事务中同时写入 `AgendaStepExecution`、
`TurnCommitReceipt`、权威状态、DomainEvent 和下一游标。恢复时以稳定 execution ID 对账：
已有 execution/receipt 时只前移游标，不重新掷骰或执行 Effect；确定性失败将 Agenda 收束为
`failed` 并释放 Turn。`AwaitPlayerInputStep` 则正常结束当前 Turn，后续由绑定同一玩家、角色、
boundary 和有限 option 的新 Turn 恢复。

#### 10.1.3 PlotThread 与玩家安全叙事

`PlotThreadState` 只保存 `locked | available | in_progress | resolved | failed`、版本和最后
transition Event。只有 Module Rule 的 `TransitionPlotThreadEffect` 可以推进；Host Proposal、
Narrator 和 Memory 都没有写入口。终态不可逆，依赖线程未解决时不能解锁后继线程。

公开 transition Event 只携带模组声明的玩家安全摘要；隐藏线程的 Event 使用 `hidden`
visibility，不进入 Narration 或 Memory。Narrator 每次调用前从最终 Engine snapshot 重新投影
非 locked 的玩家线程，本回合 transition evidence 和当前 PlotThread 状态优先于 Memory、
近期历史与已发布叙事。Memory 只能把公开 transition 投影为 `world_event`，不能反向恢复或
推进剧情线程。
