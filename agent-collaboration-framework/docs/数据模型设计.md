...
| `NarrationContext` | `player_input`, `plan_id`, `plan_goal`, `termination_status`, `completed_steps`, `player_view`, `recent_history`, `memory_context`, `focus_entity_ids`, `opening_world_time`, `allowed_evidence_refs`, `narration_evidence`, `blocked_step_goal`, `remaining_step_goals`, `player_safe_failure_reason`, `narration_retry_hint` | 单动作和 ActionPlan 共用的玩家安全叙事上下文；`memory_context` 必须与最终 PlayerView 同作用域、同 revision，持久事实仍只来自最终 PlayerView 和已提交 evidence |
