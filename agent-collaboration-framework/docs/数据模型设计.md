...
| 模型 | 字段 | 语义 |
|---|---|---|
| `HostAgentContext` | `player_input`, `player_view`, `recent_history`, `memory_context`, `keeper_capabilities` | 只接收可信 `PlayerInput`、player-safe `PlayerView`、同作用域近期历史与长期记忆；`room_id/player_id/actor_id/revision` 必须逐项一致。PR1 生产路径传入空 `memory_context`；`keeper_capabilities` 是可选的受控 `KeeperCapabilityView`（见 4.3），必须与 `PlayerView` 同 revision |
